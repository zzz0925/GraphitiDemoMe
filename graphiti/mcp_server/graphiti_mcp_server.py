...
NEO4_URI = os.getenv('NEO4J_URI')
NEO4_USER = os.getenv('NEO4J_USER')
NEO4_PASSWORD = os.getenv('NEO4J_PASSWORD')
...
async def initialize_graphiti():
    ...
    try:
        ...
        graphiti_client = Graphiti(
            uri=NEO4_URI, user=NEO4_USER, password=NEO4_PASSWORD,
            llm_client=OpenAIClient(
                config=LLMConfig(
                    api_key=API_KEY, model=DEFAULT_LLM_MODEL, small_model=SMALL_LLM_MODEL, base_url=OPENAI_BASE_URL
                )),
            embedder=OpenAIEmbedder(
                config=OpenAIEmbedderConfig(
                    api_key=API_KEY, embedding_model=DEFAULT_EMBEDDER_MODEL, base_url=OPENAI_BASE_URL
                ),
            ))
...
@router.post("/add-memory", response_model=SuccessResponse)
async def rest_add_memory(name: str,
    episode_body: str,
    group_id: str | None = None,
    source: str = 'text',
...
app = FastAPI(title="Graphiti Memory API")
app.include_router(router, prefix="/memory", tags=["Memory"])
@app.get("/sse")
async def sse(request: Request):
    """Expose the MCP SSE endpoint via FastAPI"""
    return await mcp.sse(request.scope, request.receive)
...
async def add_memory(
    ...
    print(f'Adding episode: {episode_body}')
